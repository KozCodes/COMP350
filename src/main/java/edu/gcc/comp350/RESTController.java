...
public class RESTController {
    ...
    public String getCurrentStudentName() {
        return RefactoredMain.currentStudent != null ? RefactoredMain.currentStudent.getName() : "None";
    }
    ...
    public void setCurrentSchedule(@RequestBody int scheduleId) {
        if (scheduleId >= 0) {
            for (Schedule schedule : RefactoredMain.currentStudent.getSchedules()) {
    ...
    public ResponseEntity<Schedule> getCurrentSchedule() {
        if (RefactoredMain.currentSchedule != null) {
            return ResponseEntity.ok(RefactoredMain.currentSchedule);
        } else {
        ...
        try (PreparedStatement pstmt = RefactoredMain.db.conn.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                RefactoredMain.currentStudent = new Student(
                        ...
                        rs.getString("name"),
                        rs.getString("major"),
                        new ArrayList<>(Arrays.asList(rs.getString("minor").split(" ")))
                );
...
//    }
    ...
    @PutMapping("/schedule/rename")
    public ResponseEntity<String> renameSchedule(@RequestParam int id, @RequestParam String name) {
        for (Schedule schedule : RefactoredMain.currentStudent.getSchedules()) {
        ...
        String sql = "UPDATE Schedule SET scheduleTitle = ? WHERE id = ?";
        try (PreparedStatement pstmt = RefactoredMain.db.conn.prepareStatement(sql)) {
            pstmt.setString(1, name);
            pstmt.setInt(2, id);
            pstmt.executeUpdate();
            return ResponseEntity.ok("Schedule renamed successfully.");
        } catch (SQLException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error renaming schedule: " + e.getMessage());
        }
    }

    // this code was generated by AI and slightly modified
    @GetMapping("/schedule/delete")
    public ResponseEntity<String> deleteSchedule(@RequestParam int id) {
        List<Schedule> schedules = RefactoredMain.currentStudent.getSchedules();
        for (Schedule schedule : schedules) {
            if (schedule.getId() == id) {
                RefactoredMain.currentStudent.deleteSchedule(schedule);
                break;
            }
        }
    ...
    public ResponseEntity<String> deleteCourse(@RequestParam int scheduleid, @RequestParam int courseid) {
        List<Schedule> schedules = RefactoredMain.currentStudent.getSchedules();
        for (Schedule schedule : schedules) {
            if (schedule.getId() == scheduleid) {
