...
const SchedulerPage = () => {
  ...
  const [scheduleLink, setScheduleLink] = useState(null); // New state for schedule link

  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await axios.get("http://localhost:8080/api/allAmbiguousCourses");
        const sortedCourses = response.data.sort((a, b) => a.localeCompare(b)); // Sort alphabetically
  ...
  const generateSchedule = async () => {
      try {
        ...
        if (response.data && typeof response.data === "string" && response.data.startsWith("/schedule/")) {
          setScheduleLink(response.data); // Store the schedule link
          setError("");
